...
{
    /*!
     * \brief Compute global rigid scene flow
     *         nX = Rot * X + P; nX: new X, P: translation movement, Rot:Rotation Matrix, X: original X
     *         P,O,D -----> Px,Py,Pz
     * \tparam Px: dX_x for every pixel
     * \tparam Py: dX_y for every pixel
     * \tparam Pz: dX_z for every pixel
     * \tparam P: translation movement (Global movement for the whole image)
     * \tparam O: skew symetric matrix, define how to "orbit". (Global movement for the whole image)
     * \tparam D: Depth data
     * \tparam punto: 1d array to 2d image index. it has x and y variable punto[cont].x punto[cont].y
...
{
    /*!
     * \brief Compute local rigid scene flow
     *         nX = Rot * X + P; nX: new X, P: translation movement, Rot:Rotation Matrix, X: original X
     *         P,O,D -----> Px,Py,Pz
     * \tparam Px: dX_x for every pixel
     * \tparam Py: dX_y for every pixel
     * \tparam Pz: dX_z for every pixel
     * \tparam Tx: |
     * \tparam Ty: |--- P:translation movement (Local movement for the every pixel)
     * \tparam Tz: |
     * \tparam Wx: |
     * \tparam Wy: |--- O: skew symetric matrix, define how to "orbit". (Local movement for the every pixel)
     * \tparam Wz: |
     * \tparam D: Depth data
     * \tparam punto: 1d array to 2d image index. it has x and y variable punto[cont].x punto[cont].y
     * \tparam band: good points index: 2: outlier, 1: good points 0: depth value out of range(?)
	...
	while (cont < wh) {
		if (band[cont] == 1) {
			...
			if (xx >= 0 && xx < (width[0] - 1) && yy >= 0 && yy < (height[0] - 1)) {
				...
				float i1 = P;
				Iw[pos0] = int (P); // new warped image pixel value, (warping by Px,Py,Pz)
