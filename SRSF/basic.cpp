...
void revisa(Mat &I)
{// Spanish -> revise (English)
    /*!
     * \brief convert all NaN in matrix Z to 0
     */
	int w = I.cols;
	int h = I.rows;
	float *dataI = (float*)I.data;
...
{
    /*!
     * \brief robust error function
     * \tparam I input image. normally, it is the subtraction of two other Mat
     * \tparam Iphi output robust error matrix
     * \tparam clip: clip value for discarding those element that are too big in I
     */
	int w = I.cols;
	int h = I.rows;
	float *dataI = (float*)I.data;
...
CvScalar computeColor(float fx, float fy)
{
	...
	float fk = (a + 1.0) / 2.0 * (ncols-1); //$a \in (-0.5,0.5)$ ncol=55
	int k0 = (int)fk;
	int k1 = (k0 + 1) % ncols;
	float f = fk - k0;
	...
	for (int b = 0; b < 3; b++) {
		...
		if (rad <= 1)
			col = 1 - rad * (1 - col); // increase saturation with radius = rad*col + 1 - rad
		else
			col *= .75; // out of range
...
{
    /*!
     * \brief return the max of X,Y,Z if it >= min else return min
     */
	float max;
