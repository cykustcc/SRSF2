...
{
    /*!
     * \brief create x, y index matrix grillaX and grillaY
     * \tparam grillaX
     * \tparam grillaY
     */
    //    grillaX = |1 2 3 4 ...|  grillaY = |1 1 1 1 ...|
    //              |1 2 3 4 ...|            |2 2 2 2 ...|
    //              |1 2 3 4 ...|            |3 3 3 3 ...|
    //              |...        |            |        ...|
	int w = grillaX.cols;
...
{
    /*!
     * \brief warp the original image by scene flow defined by Px,Py,Pz. output newX newY DZ:deltaZ
     * in which newX newY is the mapping matrix (each element is the (x,y) location of the pixel it should be in first image)defines the 2-D optical flow on the image, and DZ defines the depth change 
     * \tparam P:
     * \tparam O:
     * \tparam Px: Scene flow in x direction
     * \tparam Py: Scene flow in y direction
     * \tparam Pz: Scene flow in z direction
     * \tparam DZ:
     * \tparam newX:
     * \tparam newY:
     * \tparam D: element wise 1/a_Z of a_Z
     * \tparam fcx:
     * \tparam fcy:
     */
	int w = newX.cols;
...
{
    /*!
     * \brief compute jacobian matrix (2*6) for each pixel at pyr level of the image pyramid
     */
	int w = newX.cols;
	int h = newX.rows;
	int wh = w*h;
...
{    /*!
      * \brief update the translation P and Rotation Matrix O by d (delta/change of twist motion)
      */
...
{
    /*!
     * \brief construct rotation matrix R from skew symetric w by Rodrigues Formula
     */
	Mat omega = Mat::zeros(3,3,CV_32FC1);
	Mat I = Mat::eye(3,3,CV_32FC1);
	float *ww = (float*)w.data;
	float teta2 = ww[0]*ww[0] + ww[1]*ww[1] + ww[2]*ww[2]; // teta means theta here
	float teta = sqrt(teta2);
	float sinterm,costerm;

	if (teta < 0.00000149) //0.00386^2=0.00000149
	{
		sinterm = 1-teta2/6; //sin(x) = x - x^3/3!
		costerm = 1/2-teta2/24; //=(1-cos(teta))/teta^2  since cos(x) = 1 - x^2/2! + x^4/4!
	}
	else
	{
		costerm=(1-cos(teta))/teta2;
		sinterm=sin(teta)/teta;
//		printf("%f %f %f %f\n",costerm,sinterm,costerm-(1.0/2-teta2/24),sinterm-(1-teta2/6));
	}
    
    /*omega -> omega_arma and omega_exp_arma=exp(omega_arma)*/
	skew3(omega,w);
//    arma::mat omega_arma(3,3);
//    int count=0;
//    float * pomega = (float *)omega.data;
//    for (int i=0; i<3; i++) {
//        for (int j=0; j<3; j++) {
//            omega_arma(i,j)= pomega[count];
//            count++;
//        }
//    }
//    omega_arma.print("omega_arma:");
//    arma::mat omega_exp_arma;
//    omega_exp_arma = arma::expmat(omega_arma);
...
//    for (int i=0; i<3; i++) {
//        for (int j=0; j<3; j++) {
//            pR[count2] = (float)omega_exp_arma(i,j);
//            count2++;
//        }
//    }
    ...
    /*Print R using Armadillo interface*/
//    arma::mat R_arma(3,3);
//    int count2=0;
//    float * pR = (float *)R.data;
//    for (int i=0; i<3; i++) {
//        for (int j=0; j<3; j++) {
//            R_arma(i,j) = pR[count2];
//            count2++;
//        }
//    }
//    R_arma.print("R_arma:");
...
{
    /*!
     * \brief decompose rotation matrix R to skew symetric w by Rodrigues Formula
     */
	Mat s = Mat(3,1,CV_32FC1);
	float *ss = (float*)s.data;
	float *RR = (float*)R.data;

	float Tr = RR[0]+RR[4]+RR[8]; //Trace
	float Tr2 = (Tr-1)/2;
	float tetaf;
	ss[0] = RR[7]-RR[5];
	ss[1] = RR[2]-RR[6];
...
{
    /*!
     * \brief construct homogeneous representation SE (4*4 matrix) of rigid body motion from P (translation vec) and w (skew symetric)
     */
...
{
    /*!
     * \brief decompose homogeneous representation SE (4*4 matrix) of rigid body motion to P (translation vec) and w (skew symetric)
     */
	Mat R = Mat::zeros(3,3,CV_32FC1);
...
{    /*!
     * \brief track Global Rigid Motion by Niter=100 iterations
     *                                newX   newY
     *                                 |      |
     *             remap(     ,     ,newXX, newYY)
     *            -------------------------------->
     ...
     * ZY                                         TZX
     *          d^2 E_D(\xi)
     *H(i,j) = -----------------  (float H11,H12,H13,H14,H15,H16,H22,H23,H24,H25,H26,H33,H34,H35,H36,H44,H45,H46,H55,H56,H66;)
     *          d\xi_i d\xi_j
     *d\xi_i  (float d1,d2,d3,d4,d5,d6;)
     *                                           d a_gray(\xi)             d aux1(\xi)
     * J = [IJ1,IJ2,IJ3,IJ4,IJ5,IJ6]  J(1,j) = ---------------   J(2,j) = ---------------
     *     [ZJ1,ZJ2,ZJ3,ZJ4,ZJ5,ZJ6]             d \xi_j                   d \xi_j
     * \tparam gray: Intensity Image of the first frame (p_gray)
     * \tparam Z: Depth Image of the first frame (p_Z)
     * \tparam a_gray: Intensity Image of the second frame
     * \tparam a_Z: Depth Image of the second frame
     * \tparam a_D: element wise 1/a_Z of a_Z
     * \tparam Ix: x derivative of Intensity image
     * \tparam Iy: y derivative of Intensity image
     * \tparam Zx: x derivative of Depth image
     * \tparam Zy: y derivative of Depth image
     * \tparam Niter: Iterations per level
     * \tparam punto: 1d array to 2d image index. it has x and y variable, Initialize by buenPunto()
     * \tparam P: P is a 3*3 Mat and has been set to all 0
     * \tparam O: O is a 3*3 Mat and has been set to all 0
     * \tparam Px: Scene flow in x direction
     * \tparam Py: Scene flow in y direction
     * \tparam Pz: Scene flow in z direction
     * \tparam pyr: # of pyramid level
     * \tparam dd: pixel step (it considers every "stepR" pixel)
     * \tparam m_rgb: Intrinsic RGB color
     * \tparam WW: 0 ?
     * \tparam kI: Brightness term weight
     * \tparam kZ: Depth term weight
     * \tparam Npx: width of image (in the pyramid)
     * \tparam Npy: height of image (in the pyramid)
     * \tparam band: good point index: 2: out of range, 1: good 0:?
     */
	int W = 1000;
	kI = kI/W;
	kZ = kZ/W;
	float pot = pow (2.0,pyr); // in level pyr of the image pyramid, the pixel stride is pot = pow (2.0,pyr)
	int Wpot = WW*pot;
	...
	Mat H = Mat(6,6,CV_32FC1);//Hessian Matrix H_ij=d^2E/d\ksi_i\ksi_j
	float* mH = (float*)H.data;

	Mat delta = Mat(6,1,CV_32FC1);
	float* mDelta = (float*)delta.data;

	Mat dP = Mat(6,1,CV_32FC1); //delta of twist motion
	...
	while (cont > 0) {
		...
		multiply(a_Z,aux1,X,1.0/fcx);	//1.0/fcx is scale. a scalar which times the matrix multiplication
		...
		// 4) EvaLate Jacobian (dW/dP = [Jx1 0 Jx3; 0 Jy2 Jy3] dW/d\omega = [J11,J12,J13;J21,J22,J23])
		...
		for (int i = Wpot; i < Npy - Wpot; i = i+1 +dd) {
			...
			for (int j = Wpot; j < Npx - Wpot; j = j+1 +dd) {
				...
				if (band[point] == 1) {
					...
					punto2.x = (punto[point].x/pot); // x coordinate of the corresponding pixel in the resized image belongs to the pyr level of the img pyramid
					punto2.y = (punto[point].y/pot);// y coordinate of the corresponding pixel in the resized image belongs to the pyr level of the img pyramid
