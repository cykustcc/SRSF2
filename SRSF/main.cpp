...
static float maxZ = 100;        // Max depth in cm 100
static float minZ = 50;         // Min depth in cm 50
...
// Constants
static float Gauss[3][3] =  {0.0751,0.1238,0.0751,0.1238,0.2042,0.1238,0.0751,0.1238,0.0751}; //3*3 gaussian filter
int height[] = {480,240,120,60,30,15};	// Pyramid size (height)
int width[] = {640,320,160,80,40,20};	// Pyramid size (width)
//int height[] = {600,300,150,75,38};	// Pyramid size (height)
//int width[] = {800,400,200,100,50};	// Pyramid size (width)
//int height[] = {1024,512,256,128,56};	// Pyramid size (height)
//int width[] = {2048,1024,512,256,128};	// Pyramid size (width)

//int height[] = {256,128,56,28,14};	// Pyramid size (height)
//int width[] = {512,256,128,56,28};	// Pyramid size (width)
...
CvPoint2D32f *point = new CvPoint2D32f[Npoints]; //point /punto: 1d array to 2d image index. it has x and y variable punto[cont].x punto[cont].y
CvPoint2D32f *NEWpointRig = new CvPoint2D32f[Npoints]; //point correspondence in the 2nd frame(estemated by rigid motion) /punto: 1d array to 2d image index. it has x and y variable punto[cont].x punto[cont].y
CvPoint2D32f *NEWpoint = new CvPoint2D32f[Npoints];//point correspondence in the 2nd frame(estemated by non-rigid motion) /punto: 1d array to 2d image index. it has x and y variable punto[cont].x punto[cont].y

float fcx,fcy,cx,cy;
int im,im_i,im_f;


int computeSceneFlow(){
    int cont, pyr, warps;
    char name[100];
    FileStorage SFlow;
    ...
    vector < Mat > T(3 * (Npyr + 1)); // store translation motion [\ksi_1, \ksi_2, \ksi_3] of twist motion vector \ksi
    vector < Mat > W(3 * (Npyr + 1)); // store rotation motion [\ksi_1, \ksi_2, \ksi_3] of twist motion vector \ksi
    vector < Mat > SF(3 * (Npyr + 1));// scene flow non-rigid part
    vector < Mat > SFrig(3 * (Npyr + 1)); // scene flow rigid part
    ...
    int *bandOF = new int[Npoints];//good points index: 2: outlier, 1: good points 0: depth value out of range(?)
    int *bandSF0 = new int[Npoints];// not used in the warpLK
    int *bandSF1 = new int[Npoints];//good points (after first iteration) index: 2: outlier, 1: good points 0: depth value out of range(?)
    int *bandSF2 = new int[Npoints];//good points (after ConsistencySF, seems not used, 1 if diff<5) index: 2: outlier, 1: good points 0: depth value out of range(?)

    int *bandPYR = new int[Npoints];// not used
    ...
    while (Bframe < 2) {
        ...
        if (RGB.empty() || DEPTH.empty()) {
            ...
            if (im == im_i) {
                ...
                // second to the last iterations
                pyr = Npyr;
                while (pyr >= 0) {
                    // Zx Zy means depth image differential, Ix,Iy means intensity image differential, a_D means the 1/Z (element wise)
                    Sobel(a_Z[pyr], Zx[pyr], -1, 1, 0, 1); // -1 means the dst has the same depth with src, here is CV_32FC1, the last argument ksize=1 means use 1*3 kenel.
                ...
                filter2D(aux0, p_gray[0], -1, G, Point(-1, -1), 0, BORDER_DEFAULT); //-1 means default: convert src to dst in the same image depth (-1,-1) means that the anchor is at the kernel center
...
//                    printMat(p_gray[pyr],"p_gray",30);
//                    printMat(p_Z[pyr],"p_Z",30);
                ...
                while (pyr >= PyrLow) {
                    ...
                    while (ext > 0) {
                        ...
                        if (bSEL != 0) {
                            // if rigid motion
                            // bSEL !=0 means not purely non-rigid (0:non-rigid, 1:rigid, 2:non-rigid+rigid)
                        ...
                        while (warps > 0 && bSEL != 1) {
                            //warps:  Nwarps Number of alternations between GN algorithm and TV solver at each level of the pyramid
                            // bSEL !=1 means not purely rigid (0:non-rigid, 1:rigid, 2:non-rigid+rigid)
                ...
                if (PyrLow != 0) {
                    // PyrLow: Stop pyramid level of the optimization
                    // 0    -> 5
                    // fine -> coarse
                ...
                while (cont < Npoints) {
                    ...
                    if (bandSF0[cont] == 1) {
                        // if the pixel are "good points" (classified by buenPunto() function) to track (within depth assumption, )
                        // compute the pixel's new point position to NEWpointRig
                        warpLK(rx, ry, rz, point[cont], NEWpointRig[cont], a_D[0], bandSF0[cont], bandOF[cont], fcx, fcy, cx, cy);
                        
                        if (bandOF[cont] == 1) {
                            // record the 2d optical flow (w.r.t rigid scene flow) in OFxRig and OFyRig (according to the warpLK)
                            OFxRig[cont] = NEWpointRig[cont].x - point[cont].x;
                            OFyRig[cont] = NEWpointRig[cont].y - point[cont].y;
                        } 	
                        
                    } else {
                        // else if no movement, the new point is the same as the original point
                    ...
                    if (bandSF0[cont] == 1) {
                        ...
                        if (bandOF[cont] == 1) {
                        // record the 2d optical flow (w.r.t non-rigid scene flow) in OFxRig and OFyRig (according to the warpLK)
...
void seeDepthImg(){
    Mat DEPTH_IMG20(640,480,CV_32FC1);
    Mat DEPTH_IMG20_temp = imread("./Images/depth_0021.png", CV_LOAD_IMAGE_GRAYSCALE);
    DEPTH_IMG20_temp.convertTo(DEPTH_IMG20, CV_32FC1);
    cout<<DEPTH_IMG20.size().width<<" "<<DEPTH_IMG20.size().height<<endl;
    escalar(DEPTH_IMG20, 10);
    cvNamedWindow("depth");
    imshow("depth",DEPTH_IMG20);
    cvWaitKey(0);
}
void testNaN(){
    if (NaN!=NaN) {
...
void testMatrixExp(){
    ...
    omega_arma.print("omega_arma:");
    arma::mat omega_exp_arma = arma::expmat(omega_arma);
    omega_exp_arma.print("omega_exp_arma:");
}
void read_flow_and_visualize(){
    FileStorage SFlow;
    SFlow.open("./output/SFlow.xml", FileStorage::READ);
    SFlow["Frame"]>>im;
    SFlow["Rotation"]>>O;
    SFlow["Translation"]>> P;
    SFlow["SFx"]>>Vx;
...
//    read_flow_and_visualize();
    int tmp = computeSceneFlow();
    seeDepthImg();
