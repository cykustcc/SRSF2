...
void warpLK(float Px, float Py, float Pz, CvPoint2D32f punto, CvPoint2D32f &Npunto, const Mat &DD, int &bandSF, int &bandOF, float fcx, float fcy, float cx, float cy)
{
    /*!
     * \brief warp Local 
     *        generate 2d optical flow according to the 3d scene flow Px Py Pz, register pixel movement(punto to Npunto)
     *          Px,Py,Pz, punto, DD, fcx. fcy, cx, cy----> Npunto bandOF
     *
     * \tparam Px: translation movement in x direction
     * \tparam Py: translation movement in y direction
     * \tparam Pz: translation movement in z direction
     * \tparam punto: original point coordinate corresponding to the pixel index(1-# of pixels)
     * \tparam Npunto: new point coordinate correspondence
     * \tparam DD: x outlier
     * \tparam bandSF: not used
     * \tparam bandOF: new good points (if the translation Px Py Pz and Depth D are not nan)
     * \tparam fcx
     * \tparam fcy
     * \tparam cx
     * \tparam cy
	...
	if (Px != Px || Py != Py || Pz != Pz || D[pos] != D[pos])
	{
		Npunto.x = punto.x;
		Npunto.y = punto.y;
		bandOF = 0; // bad points , won't track any more
	}
	else
	{
		float xx = punto.x - cx;
		...
		bandOF = 1; // good points tot track
...
{   /*!
      * \brief Track Local Rigid using Gauss-Newton by Niter=5 iterations
      *                                newX   newY
      *                                 |      |
      *             remap(     ,     ,newXX, newYY)
      *            -------------------------------->
      ...
      * \tparam Ox: Orbit_x, wx of the skew symetric w for each pixel
      * \tparam Oy: Orbit_y, wy of the skew symetric w for each pixel
      * \tparam Oz: Orbit_z, wz of the skew symetric w for each pixel
      * \tparam RigX:
      * \tparam RigY:
      * \tparam RigZ:
		...
		for (int j=step; j<Npx-step; j = j + step + 1 ) {
			...
			if (band[point] == 1) {
				...
				while (cont > 0) {
					...
					// 3) EvaLate gradient
                    // Why not recompute the gradient in the warped image?
                    ...
                    // W = [X,Y]T P = =[Px,Py,Pz]T = [Tao_x*dt,Tao_y*dt,Tao_z*dt]T
